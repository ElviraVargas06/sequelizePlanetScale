...
export const register = async (req, res) => {
  try {
    ...
    if (error.email) {
      return res.status(404).json({
        error:
          "Correo ya se encuentra registrado, Favor ingresar otro Correo valido 😒",
      });
    }
  }
  ...
  res.status(500).json({
    message:
      "En este momento no se puede registrar en nuestro sistema, favor comunicarse con el administrador...!!",
  });
};
